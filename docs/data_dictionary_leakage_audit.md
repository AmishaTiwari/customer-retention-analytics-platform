...
| Satisfaction Score | INTEGER | Excluded (Leakage) | Fully populated across all `Customer Status` values (Stayed, Joined, Churned alike), which was the original basis for classifying this as a Feature — but that check only established availability, not independence from the outcome. A direct check of churn rate by score value found near-perfect deterministic separation: scores 1 and 2 are 100% churned, scores 4 and 5 are 0% churned, and only score 3 is mixed. This is the signature of a value assigned using knowledge of the outcome, not an independent predictive signal. |
...
- **43 columns** carried into the final modeling dataset (`mv_churn`): 41 features, plus the `customer_id` identifier and the `is_voluntary_churn` target. See `sql/04_modeling_view/01_mv_churn.sql` for the exact join and selection logic.
- **The remaining 19 raw columns are excluded from the modeling feature set**, across four distinct reasons — not one:
  ...
  - **3 post-outcome / temporal leakage columns**, which break down further into two different kinds of evidence:
    - **2 business-meaning-based exclusions, not empirically confirmed in this dataset** (see individual rationale): `Churn Score`, `CLTV`
    - **1 empirically confirmed exclusion**: `Satisfaction Score` — fully populated across all outcome values, but a direct check of churn rate by score value found near-perfect deterministic separation by outcome
...
- **Reconciliation:** 41 features + 5 identifiers + 5 target-construction-only + 3 leakage + 8 other = **62**, matching the total raw column count exactly.
